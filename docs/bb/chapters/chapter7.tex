...
\begin{mdframed}
  ...
  \node[prend] (pres)  at (0.0, 14.0)
      {Flutter Screens\\[2pt]{\normalfont\footnotesize Dashboard, PlantScan,\\AyurBot, DoshaQuiz}};
  \node[stnd]  (state) at (0.0, 10.6)
      {Riverpod Notifiers\\[2pt]{\normalfont\footnotesize Scan, Chat, Dosha, User}};
  \node[svcnd] (svc)   at (-4.0,  7.0)
      {Services Layer\\[2pt]{\normalfont\footnotesize PlantIdService\\GeminiService}};
  \node[datnd] (dat)   at ( 4.0,  7.0)
      {Data Layer\\[2pt]{\normalfont\footnotesize FirestoreService\\AuthService}};
  \node[apind] (api)   at (-4.0,  3.2)
      {External APIs\\[2pt]{\normalfont\footnotesize Plant.id v3, Gemini 2.5}};
  \node[fbsnd] (fbs)   at ( 4.0,  3.2)
      {Firebase Platform\\[2pt]{\normalfont\footnotesize Firestore, Auth}};

  %% ─── CONNECTIONS ─────────────────────────────────────────────────

  %% Mobile User → Flutter Screens
  \draw[arr] (user.south) -- (pres.north);

  %% Flutter Screens → Riverpod Notifiers
  \draw[arr] (pres.south) --
      node[lbl, right]{Consumer\\Widget}
      (state.north);

  %% Riverpod Notifiers → T-junction → Services + Data
